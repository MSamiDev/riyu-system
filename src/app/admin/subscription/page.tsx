...
const page = () => {
	...
	useEffect(() => {
		const fetchSubscriptions = async () => {
            ...
            const subs: Subscription[] = querySnapshot.docs.map(
                (doc) =>
                    ({
                        id: doc.id,
                        ...doc.data(),
		...
		<div>
			...
			<div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
				{subscriptions.map((sub, index) => (
					...
					>
						<div
							className={`h-52 flex flex-col justify-center items-center bg-blue-600 rounded-t-xl`}
						>
							<svg
								className="size-28"
								width="56"
						...
						<div className="p-4 md:p-6">
							...
							<p className="mt-3 text-gray-500 dark:text-neutral-500">
								{sub.totalMeetings} total Meetings
							</p>
						</div>
						<div className="mt-auto flex border-t border-gray-200 divide-x divide-gray-200 dark:border-neutral-700 dark:divide-neutral-700">
							<div className="w-full py-3 px-4 inline-flex justify-center items-center gap-x-2 text-sm font-medium rounded-es-xl bg-white text-gray-800 shadow-sm hover:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-white dark:hover:bg-neutral-800">
								{sub.timePeriod} Months
							</div>
							<div className="w-full py-3 px-4 inline-flex justify-center items-center gap-x-2 text-sm font-medium rounded-ee-xl bg-white text-gray-800 shadow-sm hover:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-white dark:hover:bg-neutral-800">
								{sub.price} ₨
							</div>
						</div>
					</div>
